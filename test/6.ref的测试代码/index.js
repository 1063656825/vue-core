...
// 通过添加一个 __v_isRef 属性，来标识这个对象是一个 ref 对象
/**
 * 需要校验
 * 1. 传入参数是否为一个 ref，如果是ref就不需要进行代理了
 * 2. 传入参数是否为对象
 * 3. shallowRef：创建一个浅代理对象
 */
...
// template 中为什么使用ref 不需要使用 .value 呢
// 这是因为，在template中使用ref时会被包裹一层代理
